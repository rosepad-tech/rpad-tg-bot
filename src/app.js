...
class GenericFaucetInterface {
  constructor(config) {
    ...
    this.helpMessage = `
    **Welcome to the ${process.env.FAUCET_NAME}!**

To request for tokens send the message: 

"/request ADDRESS" with your correct Oasis Emerald address!

Note: This will send 0.01 ROSE to your address. We are working to get more tokens to the faucet.`;
    // Error Messages
  ...
  async sendToken(address) {
    ...
    const RPAD_ADDRESS = "0xE7dB7564ea16491be649841B4551a6Cf4E57Cb41";
    const ERC20ABI = require('./RpadToken.json');

    const RPAD = new ethers.Contract(RPAD_ADDRESS, ERC20ABI, provider);
    return await RPAD.connect(wallet).transfer(address, ethers.utils.parseEther("1000"));
...
const config = {
  ...
  privateKey: process.env.RPAD_PRIVATE_KEY,
